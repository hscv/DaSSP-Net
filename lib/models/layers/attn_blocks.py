...
class Cross_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
        ...
        self.q_network = nn.Linear(dim, dim//self.tau, bias=qkv_bias)
        self.k_network = nn.Linear(dim, dim//self.tau, bias=qkv_bias)
        self.v_network = nn.Linear(dim, dim//self.tau, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim//self.tau, dim)
        self.proj_drop = nn.Dropout(proj_drop)

    def forward(self, q=None, kv=None, return_attention=False):
        B, N_q, C = q.shape
        _, N_kv, _ = kv.shape
        q = self.q_network(q).reshape(B, N_q, self.num_heads, C // self.tau // self.num_heads).permute(0, 2, 1, 3)
        k = self.k_network(kv).reshape(B, N_kv, self.num_heads, C // self.tau // self.num_heads).permute(0, 2, 1, 3)
        v = self.v_network(kv).reshape(B, N_kv, self.num_heads, C // self.tau // self.num_heads).permute(0, 2, 1, 3)
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N_q, C // self.tau)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x

class Cross_Attention_for_HSI(nn.Module):
    ...
    def forward(self, q=None, kv=None):
        q = self.norm1(q)
        kv = self.norm1(kv)
        x = q + self.drop_path(self.attn(q=q, kv=kv))
        return x