...
class ViPTActor(BaseActor):
    ...
    def forward_pass(self, data):
        ...
        for i in range(self.settings.num_template):
            ...
            if len(template_img_i.size()) == 5:
                out_B,in_B,H,W,C = template_img_i.size()
                template_list.append(template_img_i.view(-1,H, W, C))


        search_img = data['search_images'][0].view(-1, *data['search_images'].shape[2:]) 
        if len(search_img.size()) == 5:
            out_B,in_B,H,W,C = search_img.size()
            search_img = search_img.view(-1,H, W, C)

        if len(data['template_anno'][0].size()) == 3:
            out_B, in_B, num = data['template_anno'][0].size()
        ...
        if self.cfg.MODEL.BACKBONE.CE_LOC:
            if len(data['template_anno'][0].size()) == 3:
                ...
                box_mask_z = generate_mask_cond(self.cfg, template_list[0].shape[0], template_list[0].device,
                                        data['template_anno'][0].view(-1, num))
            else:
                box_mask_z = generate_mask_cond(self.cfg, template_list[0].shape[0], template_list[0].device,
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        # gt gaussian map
        if len(gt_dict['search_anno'][-1].size()) == 3:
            out_B, in_B, num = gt_dict['search_anno'][-1].size()
            gt_bbox = gt_dict['search_anno'][-1].view(-1, num) 
        else:
