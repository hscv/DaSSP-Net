...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val", "COCO17", "VID", "TRACKINGNET",
                        ...
                        "HSI_train_nir", "HSI_train_all", "HSI_val_nir", "HSI_train_rednir", "HSI_val_rednir"]
        ...
        settings.env.hsi_dir = '/data/lizf/HOT/Whispers2023/training/'
        if name == "HSI_train_vis":
            datasets.append(HSI3D(settings.env.hsi_dir+'HSI-VIS', split='VIS', image_loader=image_loader))
        if name == "HSI_train_nir":
            datasets.append(HSI3D(settings.env.hsi_dir+'HSI-NIR', split='NIR', image_loader=image_loader))
        if name == "HSI_train_rednir":
            datasets.append(HSI3D(settings.env.hsi_dir+'HSI-RedNIR', split='RedNIR', image_loader=image_loader))
        if name == "HSI_train_all":
            datasets.append(HSI3D(settings.env.hsi_dir+'HSI-VIS', split='VIS', image_loader=image_loader))
            datasets.append(HSI3D(settings.env.hsi_dir+'HSI-NIR', split='NIR', image_loader=image_loader))
            datasets.append(HSI3D(settings.env.hsi_dir+'HSI-RedNIR', split='RedNIR', image_loader=image_loader))
    return datasets
