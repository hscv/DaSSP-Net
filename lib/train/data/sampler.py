...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
        ...
        p_datasets = [19205, 8161, 6389] # VIS: 19205; NIR: 8161;  RedNIR: 6389
        ...
        while not valid:
            ...
            for batch_id in range(batch_size_num):
                ...
                try:
                    ...
                    select_data_arr.append(data)
                    valid = data['valid']
                except:
                    valid = False
                    raise Exception

            final_data = select_data_arr[0]
            template_img_arr = []
            template_anno_arr = []
            search_img_arr = []
            search_anno_arr = []

            for select_data in select_data_arr:
                template_img_arr.append(select_data['template_images'].unsqueeze(1))
                template_anno_arr.append(select_data['template_anno'].unsqueeze(1))
                search_img_arr.append(select_data['search_images'].unsqueeze(1))
                search_anno_arr.append(select_data['search_anno'].unsqueeze(1))
            final_data['template_images'] = torch.cat(template_img_arr, dim=1)
            final_data['template_anno'] = torch.cat(template_anno_arr, dim=1)
            final_data['search_images'] = torch.cat(search_img_arr, dim=1)
            final_data['search_anno'] = torch.cat(search_anno_arr, dim=1)
