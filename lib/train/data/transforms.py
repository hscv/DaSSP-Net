...
class Normalize(TransformBase):
    ...
    def transform_image(self, image):
        ...
        if dims[0] == 28: # FalseColor + NIR
            ...
            return torch.cat((rgb, depth), 0)
        if dims[0] == 18: # FalseColor + RedNIR
            rgb = image[:3, :, :]
            depth = image[3:, :, :]
            mean_hsi = self.mean * 9
            std_hsi = self.std * 9
...
class ToGrayscale(TransformBase):
    ...
    def transform_image(self, image, do_grayscale):
        return  image
        fc_image = image[:,:,:3].copy()
        hs_image = image[:,:,3:].copy()
        if do_grayscale:
            if torch.is_tensor(image):
                raise NotImplementedError('Implement torch variant.')
            img_gray = cv.cvtColor(fc_image, cv.COLOR_RGB2GRAY)
